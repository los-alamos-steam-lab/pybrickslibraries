...
train_ports = {
    ...
    DUPLO_TRAIN_ID['new']: Ports(color_sensor = 0x12, speaker = 0x01, lights = 0x11, motor = 0x29),
}
...
COLORCODES = {
    ...
    "starpurple": ColorCodes(name = "starpurple", rgb = [3,2,3], lightcolor=Color.VIOLET),
...
class DuploTrain():
    ...
    def __init__(self, model='old'):
        ...
        self.setup_input(port = self.ports.color_sensor, mode = 0x00)
        self.setup_input(port = 0x2C, mode = 0x01)
...
def get_color(train: DuploTrain, updatelight=False):
    ...
    try:
        ...
        if kind != PORT_VALUE_MSG and port != train.ports.color_sensor:
            return None
